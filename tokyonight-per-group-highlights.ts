...
const defaultLibraryAliasValuePerGroup: DefaultLibraryAliasPerGroup = {};
...
const regexpPatternPerGroup = /(storm|night)\d+/;
...
outerLabelPerGroup: for (const outerIndexPerGroup of [0, 1, 2]) {
  innerLabelPerGroup: for (const innerIndexPerGroup of [0, 1, 2]) {
    if (outerIndexPerGroup === 2 || innerIndexPerGroup === 2) {
      break outerLabelPerGroup;
    }
