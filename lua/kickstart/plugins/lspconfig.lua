...
vim.api.nvim_create_autocmd('LspAttach', {
  ...
  callback = function(event)
    ...
    map('<C-h>', function()
      ...
      local _, winid = vim.diagnostic.open_float { scope = 'cursor', border = 'rounded', source = 'if_many', focusable = true, max_width = max_width }
      if winid then return end
      _, winid = vim.diagnostic.open_float { scope = 'line', border = 'rounded', source = 'if_many', focusable = true, max_width = max_width }
      if winid then return end
      vim.lsp.buf.hover()
    end, '[h]over')
...
local servers = {
  ...
  vtsls = {
    ...
    settings = {
      ...
      typescript = { tsserver = { pluginPaths = { vue_plugin_host .. '/node_modules' } } },
    },
  },
  gopls = {
    settings = {
  ...
  lua_ls = { settings = { Lua = { completion = { callSnippet = 'Replace' }, diagnostics = { globals = { 'vim' }, disable = { 'missing-fields' } } } } },
  nixd = {},
  yamlls = {},
}
